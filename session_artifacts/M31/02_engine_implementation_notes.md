...
2. **Scatter symbol kind='filler'**: Changed from 'scatter' (unsupported by SymbolRegistry) to 'filler' (no payline evaluation, plugin handles scatter detection directly). This is semantically correct. If core ever adds native scatter kind support, M31 spec should be updated.
...
5. **Pre-existing test_no_machine_name_tokens_in_core_source failure**: "M15" in comments in core/engine/feature_protocol.py and core/engine/spin.py. Not caused by M31 but should be addressed by main session (rename comments to "scatter-pay trigger" or "legacy trigger" pattern).
