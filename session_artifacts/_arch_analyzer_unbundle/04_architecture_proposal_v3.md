# 04 — Architecture Proposal v3: Analyzer Unbundle (M275-driven)

> **Produced by**: arch-designer (W2) — v3 surgical fix pass
> **Date**: 2026-05-25
> **Prior version**: `04_architecture_proposal_v2.md` (v2)
> **Topic dir**: `session_artifacts/_arch_analyzer_unbundle/`
> **Inputs consumed**: v2 proposal, `05_critique_v2.md` (critic), `06_validation_v2.md` (validator)
> **Next consumer**: arch-critic (W3 v3), arch-validator (W3 v3)

---

## §0.1 Revision Log v1 → v2

*(Carried forward from v2 §0 unchanged. 24 revisions listed there.)*
...
Total revisions in §0.1 log: **24**.

---

## §0.2 Revision Log v2 → v3 **[v3 new]**

This is a surgical fix pass. Only 4 spots are patched. All other sections are carried forward verbatim from v2.

| # | Changed section | What changed | Driven by |
|---|---|---|---|
| R3-01 | §5.2 (Tier 3 jackpot) | Added `jackpot_ids_seen` raw field as secondary jackpot PID source (union semantics). Fixes M11 value regression. Added §5.2.A sub-section "Parser wiring for `jackpot_ids_seen` accumulator". Updated §12.3 M275 end-to-end walk to confirm union behaves correctly for M275. | `06_validation_v2 §3 NB1` (validator REJECT trigger) |
| R3-02 | §4.3 (new section) | Added `PipelineContext.manifest` field to the dataclass. Updated §15.5 C1 acceptance criterion: "C1 includes manifest field in PipelineContext at first construction site." | `06_validation_v2 Part C` (PipelineContext incompleteness, NB2) |
| R3-03 | §4.2 (topo-sort failure surfacing, new §4.2.X) | Added explicit surfacing path for `PluginCyclicDependencyError` / `PluginMissingDependencyError`: caught by PIA outer handler, written to `summary["analyzer_init_error"]` + stderr ERROR + non-zero rc. | `05_critique_v2 §3 NP1`, `05_critique_v2 §9 MR-v2-1` |
| R3-04 | §15.5 (C1 acceptance criteria) | Added `BankruptcySimulation` byte-identical verification row to the C1 acceptance criteria table. | `05_critique_v2 §9 MR-v2-2` |

Total surgical fixes in §0.2: **4**.
...
Phase C: _build_mechanism_registry()
  → reads: payout_id_win, payline_records, freespin_chain_spins,
            bonus_chain_lengths, jackpot_ids_seen (all from Phase A accumulators)
...
mechanism_registry = _build_mechanism_registry(
    ...
    jackpot_ids_seen=jackpot_ids_seen,           # Tier 3 raw secondary (v3: from JackpotIds field)
...
ctx = PipelineContext(
    ...
    manifest=manifest,  # v3: added per Fix 2
)
...
#### Topo-sort failure surfacing **[v3 new — Fix 3, per `05_critique_v2 §3 NP1` and `§9 MR-v2-1`]**

`PluginCyclicDependencyError` and `PluginMissingDependencyError` are programming errors (cycle or typo in `REQUIRES`). They indicate an analyzer configuration bug and must surface loudly to the operator. Per `memory/feedback_no_silent_swallow.md`, these MUST NOT be silently swallowed.

**Surfacing path**:

1. The exceptions are raised by `_topological_sort()` before any `emit()` call runs.
2. PIA's `main()` outer try/except catches them at the call site `_sorted_features = _topological_sort(_machine_features)`.
3. On catch, PIA:
   - Writes a structured diagnostic to `summary["analyzer_init_error"]` (new top-level summary field):
     ```python
     summary["analyzer_init_error"] = {
         "error_type": type(e).__name__,      # "PluginCyclicDependencyError" or "PluginMissingDependencyError"
         "message": str(e),
         "plugin_dep_graph": {                 # snapshot of REQUIRES edges at time of failure
     ...
     ```
   - Logs to stderr at ERROR level (visible to console backend subprocess logs):
     ```
     ERROR: analyzer plugin topo-sort failed: PluginCyclicDependencyError — cycle: [...]
     ```
   - Exits with a non-zero return code, so the batch runner backend marks the run as `failed` (not `completed`).
4. The console UI surfaces `summary.analyzer_init_error` if present. It is read by the frontend in the same way it reads the existing `rtp_integrity_check.summary_message` field — if the key is present in the loaded summary, a prominent error banner replaces the normal report view, showing `error_type + message`. This ensures operators discover topo-sort failures immediately rather than wondering why a run failed.

**Invariant**: Topo-sort failures are loud errors, not silent fallbacks. A cycle or missing dep in `REQUIRES` is an analyzer configuration bug and is surfaced to the operator immediately. It does NOT produce a partial report.

**Contrast with emit() errors**: `emit()` errors (a plugin crashing mid-emit) use the separate "log + set `summary["_feature_errors"][fid]`; do NOT crash" path, because a single plugin failure should not kill all other panels. Topo-sort failure is different: no plugin can emit safely if ordering is unknown, so the entire run fails.
...
- Topo-sort errors: caught by PIA outer handler; written to `summary["analyzer_init_error"]`; non-zero rc; run marked failed. See §4.2 topo-sort failure surfacing above.

### §4.3 `PipelineContext` — `manifest` field **[v3 new — Fix 2, per `06_validation_v2 Part C` NB2]**

The `PipelineContext` dataclass in §4.1 is extended with one additional field:

```python
@dataclass
class PipelineContext:
    # ... existing 7 fields from §4.1 ...

    manifest: dict[str, Any]
    # Resolved (inheritance + per-mode) manifest for this (machine, mode).
    # Source: already produced by manifest_loader.resolve_per_mode(load_manifest(machine_id), mode)
    # early in main(). Passed through to PipelineContext so plugins can read
    # manifest declarative fields at emit() time without re-loading from disk.
    #
    # Plugins that need manifest data at emit() time (e.g., payouts_by_spin_type
    # reading grid dimensions for 'shape'/'cols'/'paylines') read from ctx.manifest.
    # This avoids the alternative of stashing manifest fields in final_acc via
    # extract() (which would require every such plugin to add a no-op stash step).
    #
    # Fields commonly read by plugins:
    #   ctx.manifest.get("analyzer_features", [])
    #   ctx.manifest.get("spin_type_convention", {}).get("paid", [])
    #   ctx.manifest.get("rtp_integrity_contract", {}).get("expected_paid_st", [])
    #   ctx.manifest.get("modes", {}).get(str(mode), {}).get("grid", {})  — for shape/cols
    #   ctx.manifest.get("paylines", {})  — for paylines count
```

**Construction in PIA finalization block**: `manifest` is already resolved early in `main()` (before the merge loop). The `PipelineContext` builder in Phase D adds it directly:

```python
ctx = PipelineContext(
    # ... existing fields ...
    manifest=resolved_manifest,   # already available in main() scope
)
```

**C1 acceptance criterion update** (per §15.5): C1 includes `manifest` field in `PipelineContext` at the first construction site. After C1 ships, Phase C3's `payouts_by_spin_type.emit()` can read `ctx.manifest` for `shape`, `cols`, `paylines` enrichment without further PipelineContext changes.

**Why not stash in final_acc instead**: requiring every manifest-reading plugin to add an `extract()` stash step creates boilerplate and couples manifest structure reading to chunk-time processing, even though manifest fields are static per run. Passing manifest through PipelineContext is the simpler, more explicit mechanism — the manifest is already a local in `main()` when `PipelineContext` is constructed.

### §4.4 `SCHEMA_VERSION` semantics
...
### §5.2 Detection sources and precedence **[v2 revised; v3 Fix 1 applied to Tier 3 jackpot]**
...
- **Jackpot PID detection (Tier 3 raw) — [v3 UNION semantics, per `06_validation_v2 §3 NB1`]**:

  The jackpot PID set is the **union** of two independent detection paths. If either path identifies a PID (or any jackpot event), `jackpot_applicable = True`.

  **Path A — payout_id_win threshold**:
  ...
  4. `path_a_jackpot_pids = jackpot_candidates - scatter_marker_pids`.

  **Path B — JackpotIds raw field accumulator (secondary)**:
  The merge loop populates `jackpot_ids_seen: set[str]` by reading the `JackpotIds` field from each round response whenever it is present and non-empty (see §5.2.A for parser wiring). This accumulator captures jackpot PIDs that are signaled via `JackpotIds` but do NOT appear in `PayoutIdToWinAmount` as high-value PIDs. These PIDs may be numerically below 10000 (e.g., M11's PIDs 1102/1103/1104).
  - `path_b_jackpot_pids = jackpot_ids_seen` (the merged set from all chunks).
  - Scatter exclusion still applies: `path_b_jackpot_pids = path_b_jackpot_pids - scatter_marker_pids`.

  **Union**:
  ```python
  jackpot_pid_set = path_a_jackpot_pids | path_b_jackpot_pids
  jackpot_applicable = bool(jackpot_pid_set)
  ```
  If either source identifies a PID as jackpot, it is jackpot. Scatter exclusion is applied to both paths before union.

  `_detection_source["jackpot_pid_set"] = "tier3_raw"` (regardless of which path contributed).

  **Why this fixes M11** (`06_validation_v2 §3 NB1`): M11's jackpot PIDs (1102, 1103, 1104) appear in the `JackpotIds` raw field with confirmed non-zero win events. They are absent from `PayoutIdToWinAmount` and thus absent from `payout_id_win`. Path A produces empty set. Path B produces `{"1102", "1103", "1104"}`. Union = `{"1102", "1103", "1104"}`. `jackpot_applicable = True` (CORRECT — matches current live report value).

  **M275 under union** (`04_v2 §12.3` walk confirmed): M275's jackpot PIDs (27502, 27503, 27504) appear in `payout_id_win` with values ≥ 10000. M275 rawdata has no `JackpotIds` field populated. Path A = `{"27502", "27503", "27504"}` (after scatter exclusion of pid 666). Path B = `{}` (no `JackpotIds` events). Union = `{"27502", "27503", "27504"}`. `jackpot_applicable = True` (CORRECT — no regression, no change from v2 behavior for M275).

#### §5.2.A Parser wiring for `jackpot_ids_seen` accumulator **[v3 new — Fix 1]**

The `jackpot_ids_seen` accumulator is already listed in the `ParseState.chunk_dict` schema at `04_v2 §4.1` as a guaranteed key. In v2, it was populated by the merge loop but not used by the registry builder. In v3, the registry builder reads it as Path B.

**How the accumulator is populated**:

`parse_chunk_response()` (in PIA's merge loop) already handles the `JackpotIds` field for the existing `jackpot_spins` accumulator. The `jackpot_ids_seen` accumulator is a `set[str]` that is populated as follows in the merge loop's per-round parsing:

```python
# In parse_chunk_response() — per-round parsing (pseudocode):
jackpot_ids_raw = r.get("JackpotIds", "")   # e.g. "1104-" or "1104-1102-" or ""
if jackpot_ids_raw:
    for jid in jackpot_ids_raw.rstrip("-").split("-"):
        if jid:
            jackpot_ids_seen.add(jid)        # accumulate into chunk-level set
```

The chunk-level `jackpot_ids_seen` set is returned in the chunk dict alongside `jackpot_spins`. In the outer merge loop, chunk-level sets are unioned into the run-level `jackpot_ids_seen`:

```python
# In merge loop (pseudocode):
run_jackpot_ids_seen |= chunk_dict["jackpot_ids_seen"]
```

This accumulator is passed to `_build_mechanism_registry()` as the `jackpot_ids_seen` argument (see §4.2 Phase C pseudocode).

**Already in ParseState schema**: the `jackpot_ids_seen` key is listed in `ParseState.chunk_dict` schema at §4.1 as a guaranteed key. No schema change is needed. The change is wiring the accumulated run-level `jackpot_ids_seen` set into `_build_mechanism_registry()` as Path B input.
...
**Jackpot union within Tier 3**: Path A and Path B are both Tier 3 sub-paths. The union is computed within Tier 3 before the tier precedence check. If Tier 1 or Tier 2 has a jackpot signal, it overrides the entire union result.
...
#### Phase C1 — Mechanism Registry + emit loop fix (prerequisite) **[v2 revised; v3 Fix 2 + Fix 3 + Fix 4 land in acceptance criteria]**
...
3. `_base.py` extension: add `ParseState` dataclass, `PipelineContext` dataclass (with `manifest` field per §4.3), `DECLARED_DEPS` ClassVar. Update `emit()` signature to accept `ctx: PipelineContext` as 3rd parameter.
...
8. PIA finalization: construct `PipelineContext` from local accumulator variables before emit loop, including `manifest` field (§4.3) and `jackpot_ids_seen` passed to `_build_mechanism_registry()` (§5.2.A).
...
11. Topo-sort failure surfacing: implement the `PluginCyclicDependencyError` / `PluginMissingDependencyError` catch block per §4.2 topo-sort failure surfacing section: write `summary["analyzer_init_error"]`, log to stderr at ERROR level, exit with non-zero rc.
...
**M11 jackpot detection under C4** (per v3 Fix 1): With the `jackpot_ids_seen` Path B wiring in place from C1, M11's `machine_mechanics.jackpot.applicable` will remain `True` (correct) after C4 ships. The v3 union semantics ensure M11 is not regressed.
...
### §15.5 Per-phase acceptance criteria table **[v2 new; v3 Fix 2 + Fix 4 applied]**

| Phase | New plugins | 3 import sites | Other acceptance criteria |
|---|---|---|---|
| C1 | None (no new plugin) | Verify existing 4 plugins still imported at all 3 sites | `extract()` wired in BOTH merge paths; `PipelineContext` constructed (including `manifest` field per §4.3); topo-sort implemented; F1 sentinel assertion present; `PluginCyclicDependencyError`/`PluginMissingDependencyError` catch block writes `summary["analyzer_init_error"]` and exits non-zero; **BankruptcySimulation byte-identical check** (see row below) |
| C1 (BankruptcySimulation) | — | — | Run analyzer on M14 cached chunks before C1 commit (record `summary["player_impact"]["bankruptcy_simulation"]` section as baseline). Run again after C1 commit. Diff must be byte-identical. This proves `final_acc={}` (new call shape) vs `final_acc=None` (old call shape) produces no behavior change for BankruptcySimulation. If diff is non-empty, inspect BankruptcySimulation.emit() for `final_acc` usage and fix before proceeding to C2. |
...
| C3 | `payouts_by_spin_type` (modified, not new) | Verify existing import at 3 sites references the promoted Pattern B class (not a stale Pattern A stub) | Block F3 removed; SCHEMA_VERSION bumped to 2; `shape/cols/paylines/notes` fields present in M275 report; `shape`/`cols`/`paylines` values read from `ctx.manifest` (not stashed in final_acc) |
| C4 | `machine_mechanics` | Add `MachineMechanicsFeature` import at all 3 sites; verify `len(ALL_FEATURES) == 6` | Block F8 removed; M275 `machine_mechanics.jackpot.applicable: true`; M14 `machine_mechanics.jackpot.applicable: false`; M11 `machine_mechanics.jackpot.applicable: true` (verify no regression from v3 jackpot union semantics); 26 jackpot machines verified no L3 conflict |
...
**Consequence for v3's Fix 1** (adding `jackpot_ids_seen` Path B to registry builder): this fix edits `mechanism_registry.py`. Per §8.2, this change is NOT reflected in `effective_analyzer_version`. Operators must manually trigger regeneration for all machines after the fix. The change log entry for this fix should state: "v3 Fix 1 adds JackpotIds-field detection path; affects all machines using JackpotIds signal (at minimum M11); operator must regenerate."
...
| **M11 `machine_mechanics.jackpot.applicable`** | **C4: value preserved as `True` (correct) via v3 Fix 1 JackpotIds union** | Correct |
...
| `analyzer_init_error` summary field (new) | Present only when topo-sort fails (programming error) | Absent in healthy runs |
...
### §12.3 Finalization — MechanismRegistry build **[v2 revised; v3 union semantics confirmed for M275]**
...
- **Tier 3 raw (jackpot detection — v3 union of Path A + Path B)**:
  - Path A: Jackpot PID candidates from `payout_id_win`: `{pid for pid in payout_id_win if int(pid) >= 10000}` = `{"27502", "27503", "27504", "666"}`. Exclude scatter_marker_pids: `{"27502", "27503", "27504", "666"} - {"666"} = {"27502", "27503", "27504"}`. `path_a_jackpot_pids = {"27502", "27503", "27504"}`.
  - Path B: `jackpot_ids_seen` accumulator for M275 = `{}` (M275 rawdata has no `JackpotIds` field populated — confirmed by validator `06_validation_v2 §3 Case 8` rawdata inspection). `path_b_jackpot_pids = {} - {} = {}`.
  - Union: `jackpot_pid_set = {"27502","27503","27504"} | {} = {"27502","27503","27504"}`.
  - `jackpot_applicable = True`. `_detection_source["jackpot_pid_set"] = "tier3_raw"`.
  - **Result: identical to v2 behavior for M275. No regression. No change.**
...
`payouts_by_spin_type.emit(acc, summary, ctx)`: adds `paylines`, `shape`, `cols`, `notes` (reading grid dimensions from `ctx.manifest`). Gap #7 CLOSED.
...
**Residual Case B requiring code**: unchanged from v1 §13.1 (machines with jackpots at PID < 10000 signaled via `JackpotIds` only — now handled by v3 Fix 1; machines with novel detection signal types — still residual for truly novel signals).
...
| B — JackpotIds-field jackpots (M11-class, PIDs < 10000) | Unknown (≥1 confirmed) | YES after v3 Fix 1 | `jackpot_ids_seen` accumulator already populated; Path B detection wired |
...
### §14.9 Portrait auditability: suppressed Tier 3 evidence when Tier 1 overrides (NC1)

Per `06_validation_v2 Case 10 (NC1)`: the portrait spec (`§5.4`) does not currently require recording suppressed Tier 3 evidence when a Tier 1 `mechanism_overrides` entry takes precedence. An operator who set `mechanism_overrides: {jackpot_applicable: false}` would see the override applied but not see the Tier 3 evidence that raw data has changed. Recording suppressed evidence in the portrait for auditability is explicitly deferred: the portrait structure (`§5.4`) is not changed in v3. This is a Phase 3 operator-UX enhancement, not a correctness concern.
...
## §16 Open Questions Surviving v3 **[v2 new; v3 status noted]**

Items that the designer cannot resolve without external input.

**v3 status note**: v3 fixes the two items that the validator (REJECT) and critic (APPROVE-WITH-MINOR-REVISIONS) flagged as blocking or MR-level. The items below survive into v3 unchanged because they are either (a) confirmed acceptable by v2 critique/validation, (b) implementation-time discoveries that cannot be pre-specified, or (c) deferred to Phase 3.

**SQ1 — Gap #6 formula verdict**: the validator evidence strongly suggests the formula is correct and Gap #6 is a labeling issue. If the user or W3 v3 critic confirms this, Gap #6 can be closed by adding a portrait label improvement and no formula change. The proposal currently marks it as "formula investigation required." If confirmed correct, the Phase C5 deliverable for Gap #6 simplifies from "formula fix" to "portrait label improvement."

**SQ2 — `PipelineContext` field completeness for Phase C5**: the `PipelineContext` dataclass in §4.1 lists 7 fields (6 original + `manifest` added in v3 Fix 2). Are there other raw accumulator scalars that Block F5 (440 lines) or Block F9 (124 lines) panel plugins need? The designer identified these from `01_pipeline_map.md §7 Dependency E` and from the v2 validator's Part C analysis. The manifest gap identified by the validator is now addressed. If the implementer discovers additional required scalars during Phase C5, the dataclass must be extended before Phase C5 ships. This is an implementation-time discovery.

**SQ3 — Topological sort tie-breaking when feature ordering matters for performance**: the current tie-breaking rule is lexicographic by FEATURE_ID. If a future Phase adds two plugins where one is computationally cheap and the other expensive, and there's no REQUIRES dependency between them, lexicographic order may run the expensive one first. This is a non-functional concern. The designer's choice (lexicographic) is deterministic and predictable. W3 v3 can flag if this should be changed.

**SQ4 — M11-class fleet scale: how many machines use JackpotIds field with PIDs < 10000?** The taxonomy Ax5 counted 26 machines with jackpot PIDs ≥ 10000 (from `PayoutIdToWinAmount`). Machines like M11 that use the `JackpotIds` field with PIDs below 10000 are NOT in that count. The full fleet scale of M11-class machines is unknown. The v3 fix (Path B union) handles all of them correctly regardless of scale, because it uses the `jackpot_ids_seen` accumulator which is already populated fleet-wide in the merge loop. No per-machine enumeration is required for correctness — the fix is universal. The question is informational only: how many machines benefit from Path B?

---

## Summary Reference Table

| Design dimension | Decision (v3) |
|---|---|
...
| **Jackpot detection (v3)** | **UNION of Path A (payout_id_win PIDs ≥ 10000) and Path B (jackpot_ids_seen from JackpotIds raw field). M11 PIDs 1102/1103/1104 caught by Path B.** |
| `emit()` signature | `emit(self, final_acc: dict, summary: dict, ctx: PipelineContext) -> None` |
| `PipelineContext` fields | `effective_bet_for_rtp`, `total_spins`, `total_paid_sessions`, `total_paid_spins`, `clamp_pending_robots_total`, `robots_with_pending_cycle`, `mechanism_registry`, **`manifest` (v3 new)** |
| Topological sort algorithm | Kahn's BFS; cycle → `PluginCyclicDependencyError`; missing dep → `PluginMissingDependencyError` |
| **Topo-sort failure surfacing (v3)** | **Caught by PIA outer handler; written to `summary["analyzer_init_error"]`; logged to stderr ERROR; non-zero rc; run marked failed. UI shows error banner if key present.** |
...
| Gap #6 (correction formula) | Tentatively: formula is correct; portrait label improvement needed; pending W3 v3 confirmation |
...
| **BankruptcySimulation C1 verification (v3)** | **Byte-identical diff on M14 cached rebuild before/after C1 commit. C1 does not ship without this green.** |
