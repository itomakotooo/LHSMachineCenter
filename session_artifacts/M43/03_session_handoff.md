...
## Next session ground rules — coordinator-only mandate

Universal rule lives in **`slot_designer/ONBOARDING_PROCESS.md §4.0.1`** ("主 session coordinator-only 硬纪律", user-mandated 2026-05-14). Read it first.

Session 1 incident log (the 4 specific violations during M43 onboarding):

- Read M43Basic / M43Reel / M43Wheel / Caculate / Jackpot xlsx with inline Python — should have been Analyst (cross-check xlsx vs 01c rawdata-inferred mechanism)
- Mini-game token-to-multiplier sum verification (4199 records 100% match) — Analyst supplementary task
- Respin reel marginal comparison (base vs respin) — Analyst supplementary task
- xlsx surface triage (4 buckets: tunable / read-only / useless / hidden) — Analyst output

Captured for awareness; not to repeat. ONBOARDING §4.0.1 is the canonical rule + "even faster is no excuse" anti-rationalization.
