...
| Batch path missing post-inference scripts trigger | P1-B5 implementer + critic | `_batch_gen_worker.py` is 3rd callsite for post-analyzer inference scripts (paytable_shape + classifier) — but does NOT call `run_post_analyzer_inference(...)` after subprocess returns. Implementer deferred citing "different result format + worker pool semantics". | **OPEN — same consolidated follow-up as previous 2 rows**. `_batch_gen_worker.py` now has THREE known gaps (md5 filter forwarding + summary md5 patch + post-inference scripts). All three should be addressed by one follow-up ticket "bring `_batch_gen_worker.py` to parity with `_run_generate_report`". |
| Pure-function extraction (P1-C1 deferred sub-scope) | P1-C1 implementer + R3 critic | Brief P1-C1 §3 C4 listed pure-function extraction as OPTIONAL sub-scope. Implementer applied uniform `AppCacheState` pattern to all 5 mutable globals; deferred pure-function extraction (no specific functions identified as extraction targets). | **OPEN — P1-C2 ticket needed if value justifies**. Per brief §5, extraction is optional. Decision: not blocking Phase 1 close. Future P1-C2 ticket can identify specific pure functions worth extracting (e.g., `extract_mechanics_from_summary` / `extract_features_from_summary` / fingerprint helpers — already touched by P1-C1's TestPureFunctionExtraction tests but left in-place). |
