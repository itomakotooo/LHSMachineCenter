...
def _resolve_strips_path(weights_path: Path) -> Path:
    ...
    shared ``reel_strips.json``.

    New layout (Phase A — slot_designer/ARCHITECTURE.md §2):
      machines/<M>/
      ├── reel_strips.json
      └── weights/mode_<N>/weights.json

    Walk up: weights.json → mode_<N>/ → weights/ → <machine_dir>/.
    """
    # weights.json → mode_N/ → weights/ → machine/
    candidate = weights_path.parent.parent.parent / "reel_strips.json"
    return candidate
