...
Release flow (dev → console):
  ...
  2. update ``weights/<machine>/mode_<N>/weights.json`` (the active
     weights that virtual console sees; ``reel_strips.json`` at the
     machine dir carries the shared symbol layout)
