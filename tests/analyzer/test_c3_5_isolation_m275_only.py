...
This is the most important new test in C3.5. It asserts that:
  ...
  2. M275 effective_version FLIPPED to 2ef11cd69c8d (new plugin adds to hash).
  3. M14 effective_version UNCHANGED at dd2ab55ef022.
  4. M37 effective_version UNCHANGED at dd2ab55ef022.
  5. M272 effective_version UNCHANGED at dd2ab55ef022.
...
3. M275 mode 1 effective_version == '2ef11cd69c8d' (flipped by C3.5 plugin).
4. M14 mode 1 effective_version == 'dd2ab55ef022' (unchanged).
5. M37 mode 1 effective_version == 'dd2ab55ef022' (unchanged).
6. M272 mode 1 effective_version == 'dd2ab55ef022' (unchanged).
...
Bug A — M14 manifest gains multiplier_wild (must flip M14 effective_version):
    ...
    RED: test_m14_effective_version_unchanged fails (M14's version now includes
         multiplier_wild hash → different from dd2ab55ef022).
...
# C3.5 M275 effective_version — incorporates multiplier_wild plugin hash.
# Verified by implementer smoke run 2026-05-27.
_M275_C3_5_EFFECTIVE_VERSION = "2ef11cd69c8d"

# C3 effective_version for non-declaring machines (M14/M37/M272).
# These have: payouts_by_spin_type + reel_marginal_by_spin_type +
#             bankruptcy_simulation + multiplier_profile (4 plugins, no multiplier_wild).
# Verified by implementer smoke run 2026-05-27.
...
class TestM275EffectiveVersionFlipped:
    ...
    def test_m275_effective_version_is_c3_5_value(self):
        """M275 mode 1 effective_version must be 2ef11cd69c8d (C3.5 value).
        ...
        RED: M275 effective_version reverts to C3 non-M275 value (dd2ab55ef022).
...
class TestM14EffectiveVersionUnchanged:
    ...
    RED: test_m14_effective_version_unchanged fails (M14's hash now includes
         multiplier_wild → different from dd2ab55ef022).
    ...
    def test_m14_effective_version_unchanged(self):
        """M14 mode 1 effective_version must be dd2ab55ef022 (unchanged from C3).
...
class TestM37EffectiveVersionUnchanged:
    ...
    def test_m37_effective_version_unchanged(self):
        """M37 mode 1 effective_version must be dd2ab55ef022 (C3 value, unchanged).
...
class TestM272EffectiveVersionUnchanged:
    ...
    def test_m272_effective_version_unchanged(self):
        """M272 mode 1 effective_version must be dd2ab55ef022 (C3 value, unchanged).
