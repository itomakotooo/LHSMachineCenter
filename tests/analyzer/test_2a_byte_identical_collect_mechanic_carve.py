"""Phase 2a — REAL full deep-diff byte-identity regression for the collect_mechanic carve.

This is the gate-1 contract from
``session_artifacts/_impl/phase_extract_2a_collect_mechanic/brief.md`` §3:
the collect_mechanic carve (moving the dict-builder + 2 private helpers out of
``player_impact_analyzer.py`` into the plugin) MUST produce byte-identical report
*content*.  The pre-existing ``test_c5_byte_identical_unrelated_fields.py`` is
only *structural* (it asserts a handful of named fields are present) — it would
NOT catch a reordered dict key, a rewritten float expression, or a None-vs-0.0
flip inside the carved compute.  This test closes that gap with a TRUE full-JSON
...
The summary embeds 8 leaf KEY NAMES that legitimately vary and must be excluded:
  ...
  - version STAMPS (differ when *any* analyzer code changes — including this
    very carve, since base_hash shrank 960e9d18d83d -> 57fdb323585d):
...
EVERYTHING ELSE — the entire ``collect_mechanic`` dict, ``rtp``, all of
``player_impact``, ``config_md5`` / ``code_md5`` — must match the golden exactly.

The golden was generated by running the CURRENT post-carve analyzer (the
coordinator proved post-carve content == pre-carve content: M275 = 4667 leaves /
0 drift, M14 = 6864 leaves / 0 drift), then stripping the 8 varying keys.  So the
current output IS a valid pre-carve-equivalent golden.  Regenerate with::
...
``fresh_slotlab/analyzer/features/collect_mechanic.py`` emit(), e.g. change::

    "avg_spins_between_collects": (
        (total_spins / collect_count_total) ...
to::
    "avg_spins_between_collects": (
        (total_spins / (collect_count_total + 1)) ...   # BUG

RED: ``test_m275_content_byte_identical`` fails listing the drifted leaf path
     ``/collect_mechanic/avg_spins_between_collects`` (golden != produced).
Revert -> GREEN.
...
- memory/feedback_perf_claim_needs_e2e_event_stream.md
    ...
    catch PIA<->plugin field drift.  Mock nothing; run the actual CLI.
...
- memory/feedback_aggregator_parity_invariant.md
    collect_mechanic is RTP_CONTRIBUTION=False -> ``rtp`` must be unchanged;
...
def _flatten_leaves(obj: Any, path: str = "") -> dict[str, Any]:
    ...
    surfaces as a distinct path (e.g. /player_impact/payout_ids_top20[3]/win).
    """
    out: dict[str, Any] = {}
    if isinstance(obj, dict):
        for k, v in obj.items():
...
def _load_golden(machine: str) -> dict:
    path = _FIXTURES / f"{machine}_mode1_collect_mechanic_golden.json"
...
class TestM275ByteIdentical:
    ...
    def test_m275_content_byte_identical(self, m275_summary):
        ...
        INJECT-BUG (Path A): rewrite ONE moved expression in collect_mechanic.py
        emit() (e.g. change a divisor). RED: this test lists the drifted leaf path
        (e.g. /collect_mechanic/avg_spins_between_collects). Revert -> GREEN.
        See module docstring + inject_bug_evidence.md.
        """
        ...
        assert not diffs, (
            ...
            f"path(s)). The collect_mechanic carve must be byte-identical content.\n"
            + "\n".join(diffs[:50])
    ...
    def test_m275_collect_mechanic_in_golden_and_produced(self, m275_summary):
        """collect_mechanic must be present + non-trivially equal in both.
        ...
        """
        golden = _load_golden("M275")
        assert "collect_mechanic" in golden, "golden missing collect_mechanic"
        assert "collect_mechanic" in m275_summary, "produced missing collect_mechanic"
        assert golden["collect_mechanic"] == m275_summary["collect_mechanic"], (
            "collect_mechanic dict drifted between golden and produced — "
        ...
        assert golden["collect_mechanic"]["applicable"] is True
...
class TestM14ByteIdentical:
    ...
    M14 has collect_mechanic.applicable=False — it exercises the other half of
    the carved branch logic (no clamp_warning recommendation, None-paths).
    """
    ...
    def test_m14_content_byte_identical(self, m14_summary):
        ...
        here too (M14 exercises the applicable=False branch). RED -> revert -> GREEN.
        """
    ...
    def test_m14_collect_mechanic_not_applicable(self, m14_summary):
        """M14 collect_mechanic must be present, applicable=False, and equal to golden."""
        golden = _load_golden("M14")
        assert "collect_mechanic" in golden and "collect_mechanic" in m14_summary
        assert golden["collect_mechanic"] == m14_summary["collect_mechanic"]
        assert golden["collect_mechanic"]["applicable"] is False
...
class TestVaryingKeyAllowlistIsTight:
    ...
    def test_only_per_run_keys_vary_between_two_runs(self, m275_summary):
        ...
        genuinely drifting content field. (Version stamps are constant at fixed
