...
# PIA again → ccc1ecce185d (one-time fleet re-baseline, report content byte-identical).
_EXPECTED_C3_BASE_HASH = "ccc1ecce185d"  # phase 5: PIA shrank (reel_marginal_by_spin_type carve)
...
class TestBaseHashFlipExpected:
    ...
    is superseded. Closure value is ccc1ecce185d as of phase-5 (honesty-2 was
    ...
    phase-5's reel_marginal_by_spin_type dict-build carve shrank PIA again).
    ...
    def test_base_hash_matches_expected_c3_value(self):
        """base_hash must be the R-1 closure value (currently ccc1ecce185d, phase-5).
