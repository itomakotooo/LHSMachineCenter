...
class TestHashComposition:
    ...
    def test_base_version_does_not_include_features_dir(self):
        ...
        # Note: this test deliberately does NOT pin a literal base_hash value.
        # base_hash is in flux during the orchestrator rebuild; the structural
        # invariants above (registered plugins excluded from the closure; _base.py
        # and __init__.py kept in it) are the durable R-4 properties and do not
        # re-flag the whole fleet on every legitimate closure change.
