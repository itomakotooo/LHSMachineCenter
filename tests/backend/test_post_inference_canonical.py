...
class TestC2_BothCallsitesDelegateToHelper:
    ...
    def test_virtual_analyzer_imports_post_inference(self):
        """virtual_analyzer.py must reference fresh_slotlab.post_inference."""
        va_py = ROOT / "slot_designer" / "core" / "backend" / "virtual_analyzer.py"
        text = va_py.read_text(encoding="utf-8")
        uses_shared = (
            "from fresh_slotlab.post_inference import" in text
            or "from fresh_slotlab import post_inference" in text
            or "fresh_slotlab.post_inference" in text
        ...
        assert uses_shared, (
            "virtual_analyzer.py must delegate to fresh_slotlab.post_inference — "
            "old local _run_inference_scripts must become a thin wrapper."
        )
    ...
    def test_virtual_analyzer_no_longer_has_inline_inference_loop(self):
        """The old _run_inference_scripts loop in virtual_analyzer.py must be
        replaced by a delegation call."""
        va_py = ROOT / "slot_designer" / "core" / "backend" / "virtual_analyzer.py"
        text = va_py.read_text(encoding="utf-8")

        # Old pattern: _run_inference_scripts function with its own for-loop
        # over ("paytable_shape", ..., "classifier", ...) + subprocess.run inside.
        # The function can still exist as a thin wrapper; what must be gone is
        # the subprocess.run inside it.
        still_has_local_subprocess = (
            "def _run_inference_scripts" in text
            and "subprocess.run" in text
            and "paytable_shape" in text
        )
        if still_has_local_subprocess:
            pytest.xfail(
                "virtual_analyzer.py still appears to contain the old inline "
                "inference subprocess.run loop — impl-implementer may not have landed C2 yet"
            )
...
class TestC6_SubprocessModeE2E:
    """C6: virtual_analyzer subprocess triggers post-inference.

    Per memory feedback_perf_claim_needs_e2e_event_stream.md: subprocess-mode
    bugs need subprocess-mode tests. Spawn a real subprocess; observe filesystem
    signal.
    """

    VIRTUAL_ANALYZER = ROOT / "slot_designer" / "core" / "backend" / "virtual_analyzer.py"

    @pytest.mark.skipif(
        not (ROOT / "slot_designer" / "core" / "backend" / "virtual_analyzer.py").exists(),
        reason="virtual_analyzer.py not found in this checkout"
    )
    def test_virtual_analyzer_subprocess_inference_hook_fires(self, tmp_path):
        """Spawn virtual_analyzer.py --from-cache against a minimal empty fixture.
        Assert that the inference hook path is entered — observable via stderr
        mentioning 'post_inference' or 'infer_paytable', or via a diagnostic file.

        The fixture has no chunks, so the analyzer exits quickly. We set
        SLOT_SKIP_AUTO_INFER=0 to allow the hook to fire, but point scripts_dir
        at an empty dir so scripts are 'missing' — the script_missing path in
        the hook writes to stderr and the diagnostic file.

        Observable signals (any one of these is sufficient proof):
          1. stderr mentions 'post_inference' or 'infer_paytable' (script_missing log)
          2. _post_inference_failure.json appears in the output_dir
          3. Process exits without a Python traceback from the hook itself
        """
        machine = "M1sim"
        mode = 1
        ...
        env = dict(os.environ)
        # Enable the inference hook
        env.pop("SLOT_SKIP_AUTO_INFER", None)
        env["SLOT_SKIP_AUTO_INFER"] = "0"
        # Point at a virtual rawdata root with the right dir so no_rawdata_for_pair
        # guard does NOT short-circuit the hook.
        virtual_rawdata = tmp_path / "virtual_rawdata"
        machine_rawdata = virtual_rawdata / machine / f"mode_{mode}"
        machine_rawdata.mkdir(parents=True)
        env["SLOT_RAWDATA_ROOT"] = str(virtual_rawdata)

        cmd = [
            sys.executable,
            str(self.VIRTUAL_ANALYZER),
            "--machine", machine,
            "--rtp-mode", str(mode),
        ...
        try:
            ...
            pytest.skip("virtual_analyzer subprocess timed out (90s) — skip in slow CI")

        combined = proc.stdout + proc.stderr

        # Signal 1: stderr mentions inference hook
        hook_mentioned = (
            "post_inference" in combined.lower()
            or "infer_paytable" in combined.lower()
            or "verify_machine_labels" in combined.lower()
            or "inference" in combined.lower()
            or "script missing" in combined.lower()
            or "script_missing" in combined.lower()
        )

        # Signal 2: diagnostic file exists
        diag_files = list(output_dir.rglob("_post_inference_failure.json"))
        hook_wrote_diag = bool(diag_files)

        # If neither signal is present, check whether the analyzer exited before
        # reaching the inference hook (no chunks scenario).
        if not hook_mentioned and not hook_wrote_diag:
            no_chunks = (
                "no chunk" in combined.lower()
                or "0 chunks" in combined.lower()
                or "empty" in combined.lower()
                or proc.returncode not in (0, 1)
            )
            if no_chunks:
                pytest.skip(
                    "virtual_analyzer exited before inference hook "
                    "(no chunks in fixture). C6 needs a fixture with at least one chunk "
                    "for full subprocess-mode coverage. Mark as partial."
                )

        assert hook_mentioned or hook_wrote_diag, (
            f"virtual_analyzer subprocess did not show inference hook was reached. "
    ...
    @pytest.mark.skipif(
        not (ROOT / "slot_designer" / "core" / "backend" / "virtual_analyzer.py").exists(),
        reason="virtual_analyzer.py not found"
    )
    def test_virtual_analyzer_subprocess_no_hook_exception_propagates(self, tmp_path):
        """Even if the inference hook fails, virtual_analyzer must not crash with
        an unhandled exception from post_inference.py. The hook is best-effort.

        This guards the 'does NOT raise' contract in a real subprocess context.
        """
        machine = "M1sim"
        mode = 1
        chunk_dir = tmp_path / "rawdata" / machine / f"mode_{mode}"
        ...
        try:
            ...
            pytest.skip("virtual_analyzer subprocess timed out")

        # The subprocess must NOT crash with an unhandled exception traceback
        # that originates INSIDE post_inference.py itself (i.e., the helper
        # propagated instead of swallowing).
        # NOTE: the inference scripts themselves (infer_paytable.py /
        # verify_machine_labels.py) may produce their own Tracebacks when
        # rawdata doesn't exist — that is NOT a post_inference best-effort
        # violation; those tracebacks come from inside the subprocess the
        # helper spawns, not from post_inference.py itself.
        stderr = proc.stderr or ""
        # A post_inference crash looks like: File "...post_inference.py", line N
        # appearing OUTSIDE a subprocess invocation context. We distinguish this
        # from a script-internal crash by checking whether the traceback
        # references post_inference.py in a top-level frame.
        has_post_inference_toplevel_crash = (
            "Traceback" in stderr
            and "post_inference.py" in stderr
            and "run_post_analyzer_inference" in stderr
        )
        assert not has_post_inference_toplevel_crash, (
            "virtual_analyzer subprocess has unhandled exception from "
            "post_inference.run_post_analyzer_inference — best-effort contract violated.\n"
            f"STDERR: {stderr[-800:]}"
        )
