...
USE the frozen framework; do NOT develop it. Touch ONLY: `configs/machine_manifests/<M>.json`, NEW `features/*.py` plugins (auto-discovered, base-excluded), `configs/machine_round_win_rules.json`, `machine_spec.derive_analyses` wiring (base-excluded), and — **for every NEW analysis plugin — its FRONTEND ST dimension** (`src/web_console/frontend/app.js`: a `_stDim<Mechanic>` registered in `SPINTYPE_DIMENSIONS`, + `pure.js` i18n keys). A new plugin's output does NOT render by itself; without the dimension the console shows "almost no information" for that ST (missed on M43 AND M279 — artifact #5 of `docs/MACHINE_ONBOARDING.md`, not an afterthought). MIRROR a sibling dimension (`_stDimRespin`/`_stDimMinigame`/`_stDimWheel`) — read it in full first, reuse its markup/formatters/generic i18n keys (feedback_no_parallel_panel_impl). **NEVER edit a `_CLOSURE_FILES` file** (parser/round_win/report_engine/versioning/aggregator/feature_registry/…) — that flips the whole-fleet base_hash and is the framework team's job.
