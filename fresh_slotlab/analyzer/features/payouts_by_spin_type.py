"""AnalyzerFeature: payouts_by_spin_type — Pattern A (scaffolding).
...
Summary schema key: ``payouts_by_spin_type`` (already set by main()).
...
class PayoutsBySpinType(AnalyzerFeature):
    """Scaffolding feature for payouts_by_spin_type summary key.
    ...
    def emit(self, final_acc: Any, summary: dict) -> None:
        # main() already populated summary["player_impact"]["payouts_by_spin_type"]
        # inline (player_impact_analyzer.py line 4379).
        # This emit is a no-op verification: assert the key is present.
        # Wave 2d will move the actual aggregation here.
        player_impact = summary.get("player_impact") or {}
        assert "payouts_by_spin_type" in player_impact, (
            "player_impact.payouts_by_spin_type key missing from summary — "
